    ...
    for (let redirects = 0; ; redirects += 1) {
      ...
      try {
        response = await throttleExternalRequest(() => {
          // A request can wait in the global throttle queue longer than its
          // deadline. Do not invoke fetch with an already-aborted signal.
          if (controller.signal.aborted) throw new DOMException("aborted", "AbortError");
          return fetcher(canonical, { redirect: "manual", signal: controller.signal, headers: { "User-Agent": "Qafaseh-IranKetab-Preview/1.0", Accept: "text/html,application/xhtml+xml" } });
        });
      }
