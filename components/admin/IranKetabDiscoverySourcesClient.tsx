...
type PublisherStatus = {
  ...
  errors: Array<{ id: string; discoveryItemId: string; lastErrorCode: string | null; lastErrorMessage: string | null; updatedAt: string; titleHint: string | null; canonicalUrl: string }>;
...
function PublisherProgress({ status, processing, controlWorking, onControl }: { status: PublisherStatus; processing: boolean; controlWorking: boolean; onControl: (action: "PAUSE" | "RESUME" | "RETRY_FAILED") => void }) {
  ...
  return <section className="rounded-2xl border bg-card p-5 shadow-sm" aria-live="polite">
    ...
    {status.errors.length ? <div className="mt-5 grid gap-2"><p className="text-sm font-bold text-destructive">خطاهای اخیر ورود</p>{status.errors.map((error) => <div key={error.id} className="rounded-lg border border-destructive/20 bg-destructive/5 px-3 py-2 text-xs"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-bold">{error.titleHint || "کتاب بدون عنوان"}</span><span className="text-muted-foreground">{formatDate(error.updatedAt)}</span></div><p className="mt-1 font-mono text-[10px] text-destructive">{error.lastErrorCode || "IMPORT_FAILED"}</p><p className="mt-1 break-words text-muted-foreground">{error.lastErrorMessage || "جزئیات خطا ثبت نشده است."}</p></div>)}</div> : null}
